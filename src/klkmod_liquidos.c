...
void save_liquids()
{
	...
	sprintf( filename, "%sliquidos.datos", SYSTEM_DIR );
	if( ( fp = fopen( filename, "w" )) == NULL )
	{
		bug( "Cannot open liquidos.datos for writing." );
	}
	else
	{
...
void save_mixtures()
{
	...
	sprintf( filename, "%smezclas.datos", SYSTEM_DIR );
	if( ( fp = fopen( filename, "w" )) == NULL )
	{
		bug( "Cannot open mezclas.datos for writing." );
	}
	else
	{
