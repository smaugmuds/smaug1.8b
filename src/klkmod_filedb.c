...
void load_area_file( AREA_DATA *tarea, char *filename )
{
	...
	/* Rennard */
	else if ( !str_cmp( word, "AXUDAS"    ) ) load_helps   (tarea, fpArea);
...
void save_sysdata( SYSTEM_DATA sys )
{
    ...
    sprintf( filename, "%ssistema.datos", SYSTEM_DIR );
...
void load_weatherdata()
{
	...
	sprintf(filename, "%sotempo.datos", SYSTEM_DIR);
...
/* MAX_MSG = 18 - IF ADDING MESSAGE TYPES, ENSURE YOU BUMP THIS VALUE IN kallaikia.h */
