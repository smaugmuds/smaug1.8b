...
void write_clan_list( )
{
    ...
    if ( !fpout )
    {
	bug( "FATAL: cannot open clans.lista for writing!\n\r", 0 );
 	return;
    }	  
...
void write_council_list( )
{
    ...
    if ( !fpout )
    {
	bug( "FATAL: cannot open concellos.lista for writing!\n\r", 0 );
 	return;
    }	  
...
void save_member_lists( )
{
   ...
   {
      bug( "Cannot open membros.datos for writing", 0 );
      perror( SOCIAL_FILE );
      return;
   }
...
void load_member_lists( )
{
   ...
   {
      bug( "Cannot open membros.datos for reading", 0 );
      return;
   }
